...
async def inward_bulk_import(
    ...
    if not grn_file or not grn_file.filename:
        return HTMLResponse("<script>alert('GRN Upload is mandatory before recording a Bulk Inward Import.'); window.history.back();</script>")

    import os, datetime as _dt
    grn_dir = "grn_uploads"
    os.makedirs(grn_dir, exist_ok=True)
    timestamp_str = _dt.datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    safe_grn_filename = f"GRN_BULK_{timestamp_str}_{grn_file.filename.replace(' ', '_')}"
    grn_contents = await grn_file.read()
    try:
        with open(os.path.join(grn_dir, safe_grn_filename), "wb") as f:
            f.write(grn_contents)
    except Exception:
        pass
    ...
    try:
        ...
        for row in reader:
            ...
            if is_new:
                ...
                db.add(models.GRNRecord(item_id=new_item.id, quantity=quantity, uom=uom,
                    ...
                    grn_filename=safe_grn_filename, grn_filedata=grn_contents, uploaded_by_id=current_user.id))
                added_count += 1
            else:
                ...
                if not item:
                    ...
                    if is_tracked(item):
                        skipped_rows.append(f"{clean_code} — Fixed Assets/Equipments require serial numbers; use the single Inward form instead")
                        continue
                    if price > 0:
                        item.price = price
                ...
                # Mark any "Order Placed" procurement requests for this item as
                # Delivered, same as the single-record /transaction route does.
                placed_orders = db.query(models.ProcurementRequest).filter(
                    models.ProcurementRequest.item_id == item.id,
                    models.ProcurementRequest.status == "Order Placed"
                ).all()
        ...
        db.commit()
        msg = f"Bulk Inward Import Complete!\\nNew Items Added: {added_count}\\nExisting Items Updated: {updated_count}"
        if skipped_rows:
            msg += "\\nSkipped: " + " | ".join(skipped_rows[:5])
        return HTMLResponse(f"<script>alert('{msg}'); window.location='/';</script>")
    except Exception as e:
        db.rollback()
...
def grn_list(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for g in grns:
        ...
        e_item_name = _html.escape(item_name)
        e_item_code = _html.escape(item_code)
        e_uploader = _html.escape(uploader)
        e_vendor = _html.escape(vendor)
        e_grn_no = _html.escape(grn_no)
        e_challan_no = _html.escape(challan_no)
        rows += f"""
        <tr class="border-b hover:bg-slate-50 text-xs grn-row"
            data-date="{ts_iso}" data-item="{_html.escape(item_name.lower(), quote=True)}" data-code="{_html.escape(item_code.lower(), quote=True)}"
            data-vendor="{_html.escape(vendor.lower(), quote=True)}" data-uploader="{_html.escape(uploader.lower(), quote=True)}" data-grn="{_html.escape(grn_no.lower(), quote=True)}">
            <td class="p-3 text-slate-500 font-mono whitespace-nowrap">{ts}</td>
            <td class="p-3 font-semibold text-slate-800">{e_item_name}</td>
            <td class="p-3 font-mono text-slate-500">{e_item_code}</td>
            <td class="p-3 text-center font-mono font-bold text-slate-700">{g.quantity} {_html.escape(g.uom or '')}</td>
            <td class="p-3 text-slate-600">{e_vendor}</td>
            <td class="p-3 font-mono text-slate-500 text-center">{e_grn_no}</td>
            <td class="p-3 font-mono text-slate-500 text-center">{e_challan_no}</td>
            <td class="p-3 text-slate-500">{e_uploader}</td>
...
def mis_list(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for a in assignments:
        ...
        issued_to = a.issued_to or ''
        e_item_name = _html.escape(item_name)
        e_item_code = _html.escape(item_code)
        e_dept = _html.escape(dept)
        e_issued_by = _html.escape(issued_by)
        e_issued_to = _html.escape(issued_to)
        rows += f"""
        <tr class="border-b hover:bg-slate-50 text-xs mis-row"
            data-date="{ts_iso}" data-item="{_html.escape(item_name.lower(), quote=True)}" data-code="{_html.escape(item_code.lower(), quote=True)}"
            data-issued="{_html.escape(issued_to.lower(), quote=True)}" data-dept="{_html.escape(dept.lower(), quote=True)}" data-by="{_html.escape(issued_by.lower(), quote=True)}">
            <td class="p-3 text-slate-500 font-mono whitespace-nowrap">{ts}</td>
            <td class="p-3 font-semibold text-slate-800">{e_item_name}</td>
            <td class="p-3 font-mono text-slate-500">{e_item_code}</td>
            <td class="p-3 text-center font-mono font-bold text-slate-700">{a.quantity} {_html.escape(a.uom or '')}</td>
            <td class="p-3 text-slate-700 font-medium">{e_issued_to}</td>
            <td class="p-3 text-slate-500">{e_dept}</td>
            <td class="p-3 text-slate-400 text-[11px]">{e_issued_by}</td>
            <td class="p-3 text-right">
...
def rts_list(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for a in returns:
        ...
        e_item_name = _html.escape(item_name)
        e_item_code = _html.escape(item_code)
        e_returned_by = _html.escape(returned_by)
        e_recorded_by = _html.escape(recorded_by)
        e_dept = _html.escape(dept)
        rows += f"""
        <tr class="border-b hover:bg-slate-50 text-xs rts-row"
            data-date="{ts_iso}" data-item="{_html.escape(item_name.lower(), quote=True)}" data-code="{_html.escape(item_code.lower(), quote=True)}"
            data-returnedby="{_html.escape(returned_by.lower(), quote=True)}" data-uploader="{_html.escape(recorded_by.lower(), quote=True)}">
            <td class="p-3 text-slate-500 font-mono whitespace-nowrap">{ts}</td>
            <td class="p-3 font-semibold text-slate-800">{e_item_name}</td>
            <td class="p-3 font-mono text-slate-500">{e_item_code}</td>
            <td class="p-3 text-center font-mono font-bold text-slate-700">{a.quantity} {_html.escape(a.uom or '')}</td>
            <td class="p-3 text-slate-600">{e_returned_by}</td>
            <td class="p-3 text-slate-500">{e_dept}</td>
            <td class="p-3 text-slate-500">{e_recorded_by}</td>
            <td class="p-3 text-right">
...
async def items_bulk_import(
    ...
    try:
        ...
        for row in reader:
            ...
            stock_provided = any(
                clean_row.get(k) for k in ("initial_stock", "stock", "current_stock", "quantity")
            )
            try:
            ...
            if exists:
                exists.name = name
                exists.price = price
                if hasattr(exists, 'supplier') and vendor: exists.supplier = vendor
                if hasattr(exists, 'storage_site') and site: exists.storage_site = site
                # Route any stock change through the lot ledger instead of
                # overwriting current_stock directly, so sum(ItemLot.quantity)
                # stays in sync (keeps FIFO costing correct for later issues).
                # Only touch stock if the row actually supplied a stock value —
                # an update-only row (price/vendor change) shouldn't zero it out.
                if stock_provided:
                    stock_delta = initial_stock - (exists.current_stock or 0)
                    reconcile_stock_delta(db, exists, stock_delta)
                updated_count += 1
            else:
                new_item = models.Item(
                    name=name,
                ...
                if hasattr(new_item, 'supplier'): new_item.supplier = vendor or "Approved Vendor"
                if hasattr(new_item, 'storage_site'): new_item.storage_site = site or "Store Yard"

                db.add(new_item)
                db.flush()

                # Record the opening stock as a proper inward so it shows up
                # in the lot ledger and transaction/GRN history, same as a
                # normal inward transaction would.
                if initial_stock > 0:
                    add_to_lot(db, new_item, vendor or new_item.supplier, price, initial_stock)
                    db.add(models.Transaction(
                        item_id=new_item.id, type="IN", quantity=initial_stock,
                        user_id=current_user.id, total_value=float(initial_stock * price)
                    ))
                    db.add(models.GRNRecord(
                        item_id=new_item.id, quantity=initial_stock, uom=new_item.uom or "Nos",
                        received_by=current_user.full_name or current_user.username,
                        vendor=vendor or new_item.supplier, unit_price=price,
...
def add_item(
    ...
    existing_item = db.query(models.Item).filter(models.Item.item_code == clean_code).first()
    if existing_item:
    ...
    if initial_stock > 0:
        ...
        # Route the opening stock through the lot ledger (not a direct
        # current_stock assignment) so sum(ItemLot.quantity) stays correct
        # for later FIFO-costed issues.
        add_to_lot(db, db_item, db_item.supplier or "Approved Vendor", item_unit_price, initial_stock)
...
async def return_to_store(
    ...
    if tracked:
        ...
        if not asset_unit:
            return HTMLResponse("<script>alert('Selected asset unit not found.'); window.history.back();</script>")
        holder_norm = (asset_unit.current_holder or "").strip().lower()
        returned_by_norm = (returned_by or "").strip().lower()
        if asset_unit.status != "Issued" or holder_norm != returned_by_norm:
...
def edit_procurement_request(
    ...
    current_user = db.query(models.User).filter(models.User.username == session_user).first()
    if not current_user:
        return RedirectResponse(url="/login", status_code=303)
    ...
    # Only an Admin or the original requester may edit, and only while still Pending
    # (matches the Edit button's visibility rule on the dashboard).
    is_owner = req.requested_by_id == current_user.id
    if not (current_user.role == "Admin" or is_owner):
        return HTMLResponse("Access Denied: You may only edit your own requests.", status_code=403)
    if req.status != "Pending":
        return HTMLResponse("<script>alert('This request can no longer be edited.'); window.history.back();</script>")
    ...
    try:
        ...
        for row in reader:
            ...
            if not item:
                ...
                continue
            if is_tracked(item):
                errors.append(f"Skipped {item_code} — Fixed Assets/Equipments require a specific serial; use the Material Movement screen instead of bulk import")
                continue
...
@app.post("/rts/bulk-import")
async def rts_bulk_import(
    file: UploadFile = File(...),
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Bulk Return-to-Store import. Mirrors /mis/bulk-import but in the
    opposite direction: adds quantity back to the lot ledger and marks each
    MaterialAssignment row as a return. Serial-tracked Fixed Assets/Equipments
    are rejected here since a return must specify which physical unit came
    back — use the single-record Return form for those."""
    if not current_user:
        return RedirectResponse(url="/login", status_code=303)
    try:
        contents = await file.read()
        ...
        for row in reader:
            ...
            quantity_raw = clean.get("quantity", "0")
            uom = clean.get("uom", "Nos")
            returned_by = clean.get("returned_by", "").strip()
            department = clean.get("department", "General Operations").strip()
            remarks = clean.get("remarks", "").strip()

            if not item_code or not returned_by:
                errors.append(f"Skipped row — missing item_code or returned_by")
                continue
            try:
                quantity = int(float(quantity_raw))
            except ValueError:
            ...
            # Net issued to this employee = sum issues - sum prior returns,
            # same rule enforced by the single-record /material/return route.
            issued_total = db.query(models.MaterialAssignment).filter(
                models.MaterialAssignment.item_id == item.id,
                models.MaterialAssignment.issued_to == returned_by,
                (models.MaterialAssignment.is_return == False) | (models.MaterialAssignment.is_return.is_(None))
            ).all()
            returned_total = db.query(models.MaterialAssignment).filter(
            ...
            net_held = sum(a.quantity or 0 for a in issued_total) - sum(a.quantity or 0 for a in returned_total)
            if quantity > net_held:
                errors.append(f"Skipped {item_code} — {returned_by} only holds {net_held}, cannot return {quantity}")
                continue
        ...
        db.commit()
        msg = f"RTS Bulk Import: {added} return(s) recorded."
        if errors:
            msg += " Errors: " + " | ".join(errors[:5])
        return HTMLResponse(f"<script>alert('{msg}'); window.location='/material-movement';</script>")
    except Exception as e:
        db.rollback()
        return HTMLResponse(f"<script>alert('RTS Bulk Import Error: {str(e)}'); window.history.back();</script>")
...
<div class="max-w-[1600px] mx-auto p-6">
  ...
  <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
    ...
    <div class="px-5 pb-5 border-t border-dashed border-slate-200 pt-3">
      ...
      <form action="/inward/bulk-import" method="POST" enctype="multipart/form-data" class="space-y-2">
        ...
        <div>
          <input type="file" name="grn_file" required accept=".pdf,.jpg,.jpeg,.png,.xlsx,.xls,.doc,.docx"
            class="w-full bg-rose-50 border border-rose-200 text-slate-700 text-[10px] p-2 rounded-lg">
          <p class="text-[10px] text-rose-500 mt-0.5">&#9888; One consolidated GRN covering this whole batch is mandatory</p>
        </div>
        <button type="submit" class="w-full bg-emerald-600 text-white px-3 py-2 rounded-xl font-bold text-[10px]">Import</button>
      </form>
    </div>
  </div>
  ...
  <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
    ...
    <div class="px-5 pb-5 border-t border-dashed border-slate-200 pt-3">
      <label class="block font-black text-[10px] uppercase text-amber-700 tracking-wider mb-2">&#128202; Bulk Return Import (CSV)</label>
      <form action="/rts/bulk-import" method="POST" enctype="multipart/form-data" class="flex gap-2">
        <input type="file" name="file" accept=".csv" required class="w-full bg-slate-50 border border-slate-200 text-slate-600 text-[10px] p-2 rounded-xl">
        <button type="submit" class="bg-amber-500 text-white px-3 py-2 rounded-xl font-bold text-[10px] shrink-0">Import</button>
      </form>
      <p class="text-[10px] text-slate-400 mt-1">Columns: item_code, quantity, uom, returned_by, department, remarks. Fixed Assets/Equipments (serial-tracked) aren't supported here — use the form above.</p>
    </div>
  </div>
