...
# -------------------------------------------------------------
# AUTO-PATCH: add any model columns missing from existing tables.
# create_all() above only creates NEW tables — it never ALTERs
# existing ones. This adds any newly-added model columns to
# tables that already exist in production (Postgres or SQLite),
# so deploys never fail with "UndefinedColumn" again.
# -------------------------------------------------------------
def _auto_patch_columns():
    from sqlalchemy import inspect

    # Map SQLAlchemy column types -> SQL type strings per dialect
    def _col_sql_type(col):
        import sqlalchemy as sa
        if isinstance(col.type, sa.LargeBinary):
            return "BYTEA" if engine.dialect.name != "sqlite" else "BLOB"
        if isinstance(col.type, sa.Boolean):
            return "BOOLEAN"
        if isinstance(col.type, sa.Integer):
            return "INTEGER"
        if isinstance(col.type, sa.Float):
            return "FLOAT" if engine.dialect.name != "sqlite" else "REAL"
        if isinstance(col.type, sa.DateTime):
            return "TIMESTAMP" if engine.dialect.name != "sqlite" else "DATETIME"
        return "TEXT"
        ...
        for table_name, table in models.Base.metadata.tables.items():
            ...
            for col in table.columns:
                ...
                try:
                    ...
                    conn.commit()
                    print(f"[auto-patch] Added missing column {table_name}.{col.name}")
                except Exception as e:
                    print(f"[auto-patch] Skipped {table_name}.{col.name}: {e}")
